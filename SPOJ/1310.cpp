...
int main()
{
	...
	string skroty[5] =	{"","tys.","mln.","mld.","bln.",};
	string liczba;
	int t, i, x, cyfra;
	cin >> t;
	while (t--)
	{
		...
		{
			if ((x - i) % 3 == 0 && i != 0)
				if (((i>0)?liczba[i-1]:'0') != '0' ||((i>1)?liczba[i-2]:'0') != '0' ||((i>2)?liczba[i-3]:'0') != '0')
				{
					cout << ((i!=0 && (x-i)/3!=0)?" ":"");
					cout << skroty[(x - i) / 3];
				}
			...
			case 2:
				if (liczba[i] - '0' != 1)
				{
					...
					cout << ((i!=0)?" ":"") << nastki[liczba[i+1]-'0']; break;
			case 1:
				cout << ((i!=0 && liczba[i]-'0'!=0)?" ":"");
				cout << jednosci[liczba[i]-'0']; break;
			}

			if ((x - i) % 3 == 2 && liczba[i] == '1')
				++i;
		}
